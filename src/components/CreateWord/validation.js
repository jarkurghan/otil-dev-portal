...
// to-do: word validation oldinroq ham bo'lishi kerak
...
const definitionRequired = Joi.object({
    ...
    console.log(errors[0].code);
    errors[0].message = "definition is required";
    return errors;
});
...
const synonymsRequired = Joi.array()
    ...
    .error((errors) => {
        console.log(errors[0].code);
        errors[0].message = "synonym is required";
        return errors;
    });

const byDef = Joi.object({ word, language, word_group, definition: definitionRequired, example, history, synonyms: synonymsOptional });
const bySyn = Joi.object({ word, language, word_group, definition: definitionOptional, example, history, synonyms: synonymsRequired });
