...
export default function CreateUser({ open, setOpen }) {
    ...
    const Submit = () => {
        if (!/^\s*[a-z]+\s+[a-z]+\s*$/i.test(body.name)) return toast.error("Name is invalid");
        if (!/^[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@transoxiania.com$/i.test(body.email)) return toast.error("Email is invalid");
                            ...
                            >
                                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl">
                                    ...
                                    <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                        ...
                                        >
                                            {spinner ? <CircularProgress className="mr-2" size="0.9rem" /> : ""}
                                            Save
                                        </button>
                                        <button
                                            type="button"
